...
import com.jeffmony.downloader.transcode.FFmpegUtils;
...
public class M3U8MergeManager {
    ...
    private static final int MSG_CONVERT_TO_MP4 = 5;
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            switch (msg.what) {
                case MSG_MERGE_FINISHED:
                    ...
                    obtainMessage(MSG_CONVERT_TO_MP4, info).sendToTarget();
                    break;
    ...
    private void doMergeM3U8(VideoTaskItem taskItem) {
        ...
        File mergedFile = new File(parentPath, VideoDownloadUtils.MERGE_VIDEO);
        File outputFile = new File(parentPath, VideoDownloadUtils.OUPUT_VIDEO);
        M3U8MergeInfo info = new M3U8MergeInfo(taskItem.getUrl(), taskItem.getFilePath(), mergedFile.getAbsolutePath());
        info.setOutputPath(outputFile.getAbsolutePath());
        ...
        LogUtils.i(TAG, "VIDEO PATH = " + mergedFile.getAbsolutePath());
    ...
    private void convertTsToMp4(String inputPath, String outputPath) {
        VideoTranscodeManager.getInstance().transcode(inputPath, outputPath);
    }

    private void deleteM3U8File(M3U8MergeInfo info) {
        String filePath = info.getM3U8Path();
        String parentPath = filePath.substring(0, filePath.lastIndexOf("/"));
        ...
        if (parentFile.exists()) {
            for (File subFile : parentFile.listFiles()) {
                ...
                if (!VideoDownloadUtils.MERGE_VIDEO.equals(name) &&
                        !VideoDownloadUtils.OUPUT_VIDEO.equals(name) &&
